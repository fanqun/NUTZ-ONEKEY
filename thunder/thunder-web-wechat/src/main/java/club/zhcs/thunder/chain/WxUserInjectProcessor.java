...
//			CustomerService customerService = ac.getIoc().get(CustomerService.class);
//			Mvcs.getReq().getSession().setAttribute(Application.SessionKeys.USER_KEY, customerService.findByField("openid", openid));
			doNext(ac);
			return;
		}
		...
		String wechatInterface = "https://api.weixin.qq.com/sns/oauth2/access_token?appid=" + props.get("appid") + "&secret=" + props.get("appsecret") + "&code=" + code
...
//		CustomerService customerService = ac.getIoc().get(CustomerService.class);
//		Mvcs.getReq().getSession().setAttribute(Application.SessionKeys.USER_KEY, customerService.findByField("openid", data.getString("openid")));
		doNext(ac);
	}
