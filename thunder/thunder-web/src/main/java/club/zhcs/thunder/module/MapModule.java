...
public class MapModule extends AbstractBaseModule {
	...
	public Result show(int mid) {
		String url = String.format("http://api.map.baidu.com/location/ip?ak=%s&ip=%s&coor=bd09ll", config.get("baidu.map.key"), _ip());
		Response response = Http.get(url);
		if (response.isOK() && Lang.map(response.getContent()).getInt("status") == 0) {
			NutMap data = Lang.map(response.getContent());
			return Result.success().addData("ak", config.get("baidu.map.key")).addData("lng", data.getAs("content", NutMap.class).getAs("point", NutMap.class).get("x"))
					.addData("lat", data.getAs("content", NutMap.class).getAs("point", NutMap.class).get("y"));
		}
		return Result.success().addData("ak", config.get("baidu.map.key")).addData("lng", 106.582656).addData("lat", 29.530382);
	}
}